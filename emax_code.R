...
bcrm<-function(stop=list(nmax=NULL,nmtd=NULL,precision=NULL,nmin=NULL),data=NULL,p.tox0=NULL,sdose=NULL,dose=NULL,ff,prior.alpha,cohort=3,target.tox,constrain=TRUE,sdose.calculate="mean",pointest="plugin",tox.cutpoints=NULL,loss=NULL,
               start=NULL,simulate=FALSE,nsims=1,truep=NULL,threep3=FALSE, cohort.control=1,
               method="exact",burnin.itr=4000,production.itr=10000,bugs.directory="c:/Program Files/WinBUGS14/",plot=FALSE,seed=NULL, quietly=FALSE, file=NULL,N,tox,notox){
  ...
  if((!ff %in% c("ht","logit1","power","logit2","emax"))) stop("ff must be one of `ht', `logit1', `power', `logit2' or `emax'")
  
  if(ff=="emax" & method=="exact") warning("Exact method slow for 2-parameter model, suggest using rjags (MCMC)")
  ...
  if(missing(sdose)){
    alpha.prior.plug<-if(prior.alpha[[1]]==1){
      ...
      if(sdose.calculate=="mean"){c(0.08,1,0.92,1)} else {c(0.08,1,0.92,1)}
    } 		
  ...
  if(method=="exact" & ff=="emax"){ 
    method<-"exact.sim"
    if(plot){
      plot<-FALSE
      warning("Plot function not available for exact computation of 4-parameter model")
    }
  }
  k<-length(sdose)
  sim<-1	
  ...
  while(sim<=nsims){
    ...
    while(!stopped){
      ###or put ncurrent here
      if (ndose[[1]]==1 & ncurrent<stop[[1]] ){
        ndose[[1]]<-2
      } else{}
      ncurrent<-ncurrent+cohort+cohort.control
      ...
      if(!simulate){
        ...
        y.control<-rbinom(cohort.control,1,truep[1]) 
        new.notox[ndose[[1]]]<-new.notox[ndose[[1]]]+(cohort-sum(y))
        new.tox[ndose[[1]]]<-new.tox[ndose[[1]]]+sum(y)
        new.notox[1]<-new.notox[1]+(cohort.control-sum(y.control))
        new.tox[1]<-new.tox[1]+sum(y.control)
      }
      currentdata<-data.frame(patient=(ncurrent-cohort+1):ncurrent,dose=rep(current,cohort),tox=y)
      newdata<-rbind(newdata,currentdata)		
      currentdata<-data.frame(patient=(ncurrent-cohort.control+1):ncurrent,dose=rep(1,cohort.control),tox=y.control)
      newdata<-rbind(newdata,currentdata)	
...
{    
  ...
  mydata <- list(N1 = k, s = datas,n = datan,d = datad)
  ...
  inits.list<-if (ff=="emax")
    list(list(alpha=c(0.08,1,0.92,1),.RNG.seed=sample(1:1e6, size=1),.RNG.name="base::Wichmann-Hill"),list(alpha=c(0.08,1,0.92,1),.RNG.seed=sample(1:1e6, size=1),.RNG.name="base::Wichmann-Hill"))
  #list(list(alpha=c(0.08,1,1,0.5),.RNG.seed=sample(1:1e6, size=1),.RNG.name="base::Wichmann-Hill"),list(alpha=c(0.08,1,1,0.5),.RNG.seed=sample(1:1e6, size=1),.RNG.name="base::Wichmann-Hill"))
  ...
  if(ff=="emax"){
    t<-cbind(c(tt$alpha[1,,]),c(tt$alpha[2,,]),c(tt$alpha[3,,]),c(tt$alpha[4,,]))
  } else {   t<- c(tt$alpha) }
  return(t)
}
...
which.f <- function( ff ) {
  return( switch(ff,
                 ...
                 emax=function(dose,alpha) {alpha[,1]+ (dose^alpha[,2]*alpha[,3])/(dose^alpha[,2]+alpha[,4]^alpha[,2])} ))
}
...
find.x <- function( ff, ptox, alpha ) {
  if ( ff == "ht" ) {
    ...
    x <- ((-alpha[1]*(alpha[4]^alpha[2])+(alpha[4]^alpha[2])*ptox)/(alpha[3]-ptox+alpha[1]))^(1/alpha[2])
  }
  return( x )
}
...
plot.bcrm<-function(x,file=NULL,each=FALSE,trajectory=FALSE,...){
  ...
  if(trajectory){
    ...
    if(!each){
      if(x$method %in% c("exact","exact.sim") & x$ff=="emax"){
...
getprior <- function(prior.alpha, n) {
  ...
  if ( type==1 ) {
    ...
    first<-rbeta(n, 0.08*10,10-0.08*10)
    #second<-rexp(n,2/3)
    second<-rgamma(n,1,rate=1)
    third<-runif(n,0,1-0.08)
    #fourth<-rexp(n,1)
    fourth<-rgamma(n,0.4,rate=0.4)
    prior<-cbind(first,second)
    prior<-cbind(prior,third)
    prior<-cbind(prior,fourth)
  }
  return (prior)
}
...
TwoPLogisticLogNormal<-function(){
  for (i in 1:N1){
    s[i] ~ dbin(p[i], n[i])		
    p[i] <- alpha[1]+ (pow(d[i],alpha[2])*alpha[3])/(pow(d[i],alpha[2])+pow(alpha[4],alpha[2]))
  }
  alpha[1]~dbeta(0.08*10,10-0.08*10)
  alpha[2]~dgamma(1,1)
  alpha[3]~dunif(0,1-alpha[1])
  alpha[4]~dgamma(0.4,0.4)
}

benchmark<-function(P,N,target,sim){
  M<-length(P)
  result.table<-mat.or.vec(2,M)
  rownames(result.table)<- c("true","Optimal")
  result.table[1,1:M]<-P
  estimate.optimal<-mat.or.vec(sim,M)
  res.all<-mat.or.vec(sim,M)
  dis<-mat.or.vec(M,1)
  tab<-mat.or.vec(N,M)
  
  for (z in 1:sim){
    realiz<-runif(N, min = 0, max = 1)
    ...
    res.all[z,which.min(dis)]<-1
    estimate.optimal[z,]<-R
    
  }
  result.table[2,1:M]<-colSums(res.all)/sim*100
  y<-estimate.optimal
  return(y)
}
