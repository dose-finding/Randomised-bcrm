...
threshold<-0.05
ff<-"emax"
f<-emax.model<-function(dose,alpha) {alpha[,1]+ (dose^alpha[,2]*alpha[,3])/(dose^alpha[,2]+alpha[,4])}
...
bcrmodel<-bcrm(stop=list(nmax=Ntotal),p.tox0=p.tox0
               ...
               ,constrain=FALSE,pointest="mean",method="rjags",start=2,simulate=TRUE,nsims=nsims,truep=truep)
print.bcrm.sim(bcrmodel)
...
mean(result.probability>0.9)

set.seed(101)
p.opt<-benchmark(P=truep,N=Ntotal,target=target.tox,sim=nsims)
